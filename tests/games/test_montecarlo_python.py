...
class TestMontecarloPython(unittest.TestCase):

    def _runner(self, simulation, my_cards, cards_on_table, players, opponent_range=1.0):
    ...
    def test_montecarlo1(self):
        ...
        avg, stdev = self._runner(simulator, my_cards, cards_on_table, players)
        self.assertTrue(abs(avg - expected_results) < 2)
        self.assertTrue(abs(stdev) < 2)
